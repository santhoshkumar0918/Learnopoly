      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="space-y-8">
          {posts?.map((post) => (
            <div
              key={post.id}
              className="bg-gray-700 rounded-xl p-6 space-y-4 transform transition-all hover:scale-102 duration-300 ease-in-out"
            >
              <div className="flex items-start space-x-3">
                <img
                  src={user?.imageUrl}
                  ...
                  {post.image_url && (
                    <div className="relative mt-3">
                      <img
                        src={`${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/post-images/${post.image_url}`}
                        alt="Post image"
                        className="w-full max-h-[400px] object-cover rounded-lg transition-transform duration-500 transform hover:scale-105"
                      />
                    </div>
                  )}
                </div>
